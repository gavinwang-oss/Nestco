...
- **Framer Motion** — panel/grid transitions in browse (swipe gestures were removed)
...
| Route | File | Purpose |
|---|---|---|
| `/` | `app/page.tsx` → `components/BrowseExperience.tsx` | **The homepage and main page** — listing grid, AI search/chat panel, detail panel. URL stays at root (no `/browse` suffix). `app/page.tsx` just renders `<BrowseExperience />` (the implementation, a client component). Shows profile completion banner if profile is incomplete. Expired listings filtered out. Includes `<OnboardingTooltip>`. Reads `?listing=<id>` to open a detail panel directly. Unauthenticated visitors are sent to `/login` by `RouteGuard`. (Swipe gestures and compare mode were removed.) |
...
### `profiles`
User profiles. Fields: `user_id`, `name`, `age`, `major`, `year_in_school`, `race`, `gender`, `bio`, `avatar_url`, `include_demographics`. The current UI no longer collects race or exposes the include-demographics toggle. RLS: readable by all authenticated users (needed for name reveal after match).
...
- AI prompts and UI copy must not use race, ethnicity, cultural background, religion, national origin, or similar protected traits as housing matching signals.
...
The chat API is a pure search/rank wrapper — it returns raw JSON (not markdown). Claude (`claude-haiku-4-5`) returns:
```json
{ "content": "...", "rankedIds": [], "scores": {}, "suggestedListingId": null }
```
`rankedIds`/`scores` reorder and score the listing grid; `suggestedListingId` optionally highlights one listing. The route strips markdown code blocks if Claude wraps the JSON anyway. The AI prompt instructs plain text only (no `**bold**`, no `-` bullets). The AI does NOT drive UI actions or draft messages — that protocol was removed.
...
1. User clicks "Message the lister" on a listing detail panel → opens `MessageModal`
2. User writes their own message and sends → inserts into `messages` with `recipient_id = listing.user_id`
3. A fire-and-forget email notification is sent to the lister via `/api/notify/message`
(AI-drafted intro messages were removed — users write their own.)
...
- `RouteGuard.tsx`:
  - `PUBLIC_PATHS` (no login): `/login`, `/dev-login`, `/tos`, `/about`, `/auth/callback`.
