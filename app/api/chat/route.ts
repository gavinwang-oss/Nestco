...
export async function POST(req: NextRequest) {
  try {
    ...
    const viewingSummary = selectedListing
      ? `User is currently viewing: ${selectedListing.address} (ID ${selectedListing.id}, $${selectedListing.price}/mo, ${selectedListing.type})`
...
- scores maps listing IDs to 0-100 match percentages. When the user mentions ANY specific criteria (price, dates, type, gender preference, furnishing, utilities, roommates, distance, pets, parking, etc.), score EVERY SINGLE listing ID — do not omit any. IMPORTANT: if the user asks for something you cannot filter by (e.g. number of windows, color, specific view), still score and rank ALL listings based on every other criterion you CAN evaluate. Never skip scoring just because part of the request is impossible to fulfill. Listings that match well score 80-100, partial matches 40-79, incompatible listings score 10-30. Never omit a listing from scores once criteria exist. Use graduated scoring for price (full score within 10% of budget, partial for slightly over) and distance. Only hard-zero a listing if it is completely incompatible (e.g. wrong gender for a gender-locked listing). If the user has not mentioned any criteria at all, set scores to {}.
